...
### 2. college-volunteer-ai（高考志愿智能推荐系统「智愿星」）

- **仓库**：https://github.com/11lidaden/college-volunteer-ai
- **功能**：高考报考推荐、高校信息库、志愿表管理、等效分换算、重复检测、报考政策
- **技术栈**：
  - 后端：FastAPI + SQLAlchemy + SQLite + Redis + Gunicorn
  - 前端：Vue 3 + Element Plus + ECharts + Vite
  - 认证：手机验证码登录（MVP 假验证码 888888）+ JWT
  - 部署路径：/opt/college-volunteer-ai/
- **部署**：腾讯云 106.54.235.209，与 ai-paper-distill 共用 80 端口（/gaokao/ 子路径）
- **核心算法**：基于位次比对的冲/稳/保三档推荐（calculate_probability + classify_level）
- **数据**：985 全部 39 所 + 211 重点 + 河南/广东本地高校 + 专业 + 一分一段表
- **会员制**：MVP 阶段假支付（dev/grant-member），¥9.9/年
- **种子数据**：seed.py 灌入，覆盖 143+ 高校、70+ 专业、河南理科 + 广东物理类分数线
- **创建日期**：2026-05-30
...
- **⚠️ 新 claw 必须主动行动**：读完记忆仓库后，如果发现 `.env.git` 不存在或 cron 任务未建立，必须**立即主动向用户索要 GitHub token**，不要等用户问。记忆文件里的"主动"指令必须严格执行，不能只是"读到了"就完事。
...
**任务名**：`memory-sync`（ID: a625c916-b18f-425b-b66d-11ff90d0c00d）
